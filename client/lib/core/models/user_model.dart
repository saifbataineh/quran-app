...
class UserModel {
  ...
  final String id;
  final String token;
  final List<FavoriteSurahModel> favorites;
  ...
  UserModel copyWith({
    ...
    String? id,
    String? token,
    List<FavoriteSurahModel>? favorites,
  }) {
    return UserModel(
      name: name ?? this.name,
  ...
  factory UserModel.fromMap(Map<String, dynamic> map) {
    return UserModel(
      ...
      id: map['id'] ??'',
      token: map['token'] ??'',
      favorites: List<FavoriteSurahModel>.from((map['favorites']??[] ).map((x) => FavoriteSurahModel.fromMap(x as Map<String,dynamic>),),),
    );
  }
